...
# coding: utf-8
"""
Qwen3-VL-8B-Instruct on COCO: load model, run VQA, GradCAM attention, and bbox detection.
Converted from Qwen3-VL-8B-Instruct_on_Coco.ipynb
"""
# pip: torchvision qwen-vl-utils accelerate transformers pillow
...
# Use eager attention so output_attentions=True is supported (sdpa does not return attentions)
...
_output_base = os.path.abspath(os.path.join(_script_dir, "output"))
...
def compute_attention_weights_map(model, processor, image, prompt, sharpen_power=2.0):
    """
    Attention from last token to image tokens only (no grad). Uses the same
    detection prompt so attention is concentrated on the objects we ask about.
    sharpen_power > 1 suppresses background and strengthens key regions.
    """
    messages = [{
        "role": "user",
        "content": [
    ...
    if outputs.attentions is None or len(outputs.attentions) == 0:
        raise ValueError("model returned no attentions (ensure set_attn_implementation('eager') was called)")
    # Last layer: (batch, heads, seq_len, seq_len)
    last_attn = outputs.attentions[-1]
    image_grid_thw = inputs.get("image_grid_thw")
    if image_grid_thw is None or image_grid_thw.numel() < 3:
        raise ValueError("processor did not return image_grid_thw in inputs")
    t, h, w = int(image_grid_thw[0, 0].item()), int(image_grid_thw[0, 1].item()), int(image_grid_thw[0, 2].item())
    n_patches = t * h * w
    seq_len = last_attn.shape[-1]
    if n_patches > seq_len:
        raise ValueError(f"n_patches ({n_patches}) > seq_len ({seq_len}); image token layout may differ")
    # Attention from last token to image tokens (Qwen3-VL may put image tokens at start or end)
    ...
    importance_map = attn_to_image.reshape(t, h, w)
    if t > 1:
        importance_map = importance_map.mean(axis=0)
    else:
...
def compute_gradcam_attention(model, processor, image, question):
    """
    Compute GradCAM importance over image patches (same prompt as detection for
    object-focused map). Backprop from next-token logits.
    """
    messages = [{
        "role": "user",
        "content": [
    ...
    t, h, w = int(image_grid_thw[0, 0].item()), int(image_grid_thw[0, 1].item()), int(image_grid_thw[0, 2].item())
    n_patches = t * h * w
    if importance_per_patch.size < n_patches:
        return None
    importance_per_patch = importance_per_patch[:n_patches]
    importance_map = importance_per_patch.reshape(t, h, w)
    if t > 1:
        importance_map = importance_map.mean(axis=0)
    else:
        importance_map = importance_map.squeeze(0)
...
Only return the coordinates, nothing else."""
    ...
    try:
        ...
        lines.append("")
        lines.append("Qwen3-VL-8B-Instruct PREDICTIONS (prompt = GT labels for this image)")
        lines.append("-" * 40)
